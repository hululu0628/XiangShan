 ...
 * IO of VTypeGen
 */
class NewVTypeGenIO(implicit p: Parameters) extends XSBundle {
...
class NewVTypeGen(implicit p: Parameters) extends XSModule {
  val io = IO(new NewVTypeGenIO())
  ...
  io.vtype(0) := Mux(isVsetiVec(0), vtypeNewVec(0), vtypeSpec)
  io.specvtype(0) := vtypeSpec
  for(i <- 1 until DecodeWidth) {
    io.vtype(i) := Mux(isVsetiVec(i), vtypeNewVec(i), io.vtype(i-1))
